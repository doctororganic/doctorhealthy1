...
func newAuthMetrics() *AuthMetrics {
	...
	prometheus.MustRegister(m.BlockedRequests)
	prometheus.MustRegister(m.ActiveRateLimiters)
	prometheus.MustRegister(m.JWTValidations)
	prometheus.MustRegister(m.APIKeyValidations)
	prometheus.MustRegister(m.SuspiciousActivity)
