...
async function loadWorkoutData() {
    try {
        ...
        // Use safe JSON parsing if error handler is available
        if (window.errorHandler && typeof window.errorHandler.safeJSONParse === 'function') {
            const parseResult = window.errorHandler.safeJSONParse(data);
            if (parseResult.success) {
                workoutData = parseResult.data;
            } else {
                throw new Error('Invalid JSON format in workout data');
            }
        } else {
